...
public class ShopOrderDAO {
	private static ShopOrderDAO sDAO;
	private ConnectionPool cp = ConnectionPool.getInstance();

	// SQL 쿼리문 정의
	private final String SELECT_SQL = "SELECT RNUM, NO, TYPE, PRODUCT_NO, NAME, PRICE, AMOUNT, TITLE, CONTENT, TITLEURL, SUBDATE from(\r\n"
			+ "select  ROWNUM RNUM, NO, TYPE, PRODUCT_NO, NAME, PRICE, AMOUNT,TITLE, CONTENT, TITLEURL, SUBDATE FROM (\r\n"
			+ "SELECT s.NO, s.TYPE, s.PRODUCT_NO, p.NAME, p.PRICE, p.AMOUNT, \r\n"
			+ "s.TITLE, s.CONTENT, i.URL AS TITLEURL, s.SUBDATE FROM (\r\n"
			+ "SELECT * FROM SHOP WHERE TYPE = ? \r\n"
			+ "ORDER BY NO DESC) s LEFT JOIN PRODUCT p ON s.PRODUCT_NO = p.NO \r\n"
			+ "LEFT JOIN (SELECT URL, SHOP_NO FROM SHOP_IMG \r\n"
			+ "WHERE TYPE = 'TITLE') i ON s.NO = i.SHOP_NO order by s.no\r\n"
			+ "))WHERE RNUM BETWEEN ? AND ? ORDER BY RNUM DESC";
	
	private final String SELECT_BY_TITLE_SQL = "SELECT RNUM,NO, TYPE, PRODUCT_NO, NAME, PRICE, AMOUNT, TITLE, CONTENT, TITLEURL, SUBDATE from(\r\n"
			+ "select ROWNUM RNUM, NO, TYPE, PRODUCT_NO, NAME, PRICE, AMOUNT,TITLE, CONTENT, TITLEURL, SUBDATE FROM (\r\n"
			+ "SELECT s.NO, s.TYPE, s.PRODUCT_NO, p.NAME, p.PRICE, p.AMOUNT, \r\n"
			+ "s.TITLE, s.CONTENT, i.URL AS TITLEURL, s.SUBDATE FROM (\r\n"
			+ "SELECT * FROM SHOP WHERE TYPE = ? AND TITLE LIKE '%'||?||'%' ORDER BY NO DESC) s \r\n"
			+ "LEFT JOIN PRODUCT p ON s.PRODUCT_NO = p.NO LEFT JOIN (\r\n"
			+ "SELECT URL, SHOP_NO FROM SHOP_IMG WHERE TYPE = 'TITLE') i ON s.NO = i.SHOP_NO order by s.no)) WHERE RNUM BETWEEN ? AND ? ORDER BY RNUM DESC";
	...
	private final String SELECT_LIST_SQL = "SELECT * FROM (SELECT ROWNUM AS RNUM , s.NO ,s.type,s.PRODUCT_NO,p.name,p.price,p.amount,s.TITLE,s.CONTENT,url as titleurl,s.SUBDATE FROM SHOP s left join\r\n"
			...
			+ "(select url,shop_no from SHOP_IMG where type='TITLE') i on s.no=i.shop_no\r\n"
			+ "ORDER BY S.SUBDATE) WHERE no = ?  ORDER BY RNUM DESC";
	private final String SELECT_LAST_NO_SQL = "SELECT * FROM (SELECT * FROM SHOP_ORDER ORDER BY NO DESC) WHERE ROWNUM=1 ";
	...
	private final String INSERT_SQL = "INSERT INTO SHOP_ORDER VALUES((SELECT NVL(MAX(NO), 0) + 1 FROM SHOP), ?, ?, ?, ?, SYSDATE,?)";
	...
	public static ShopOrderDAO getInstance() {
		if (sDAO == null) {
			synchronized (ShopOrderDAO.class) {
				if (sDAO == null) {
					sDAO = new ShopOrderDAO();
				}
			}
		}
		return sDAO;
