...
public class ShopFindSelectAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws IOException {
    	...
    	if (request.getParameter("findText") != null) {//pageNum request 값이 있으면 받아온다
    		findText = request.getParameter("findText");
    	}
    	//데이터베이스 에서 가져오는 값
    	int recordCount = pDAO.selectRecordByTitleDB(findText,svo); //전체 리스트 개수 findText값이 있을시 해당 항목으로 변경
		...
		ArrayList<ShopVO> sList = pDAO.selectByTitleDB(findText,startListNum, endListNum,svo);//원하는 구간의 db를 출력
		...
		ActionForward forward = new ActionForward("/home/shop/shopFindPage.jsp", false);
		return forward;
	}
