...
<link rel="stylesheet" href="${pageContext.request.contextPath}/home/shop/css/shopListPage.css" />
<c:if test="${empty svo}">
...
<div align="center">
    ...
    <div class="backButton">
        <button type="button" onclick="location.href='<c:url value='/shopSelect.do?type=${param.type}' />'">목록으로</button>
    </div>
    <c:if test="${cvo.id=='admin'}">
    <div class="backButton">
        <button type="button" onclick="location.href='<c:url value='/shopListDelete.do?no=${param.no}&type=${param.type}' />'">삭제하기</button>
    </div>
</c:if>
    <!-- 상품 정보 섹션 -->
    <section class="productInfoSection">
        ...
        <div class="basicInfo">
            <h2>${svo.title}</h2>
            <p class="price"><fmt:formatNumber value="${svo.price}" type="currency" /></p>
            ...
            <input value="${svo.no}" type="hidden" name="no">
            <input value="${svo.type}" type="hidden" name="type">
