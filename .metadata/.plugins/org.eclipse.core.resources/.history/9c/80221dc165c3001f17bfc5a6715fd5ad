...
public class ProductFindSelectAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		// 데이터베이스 에서 가져오는 값
		int recordCount = pDAO.selectRecordByTitleDB(findText); // 전체 리스트 개수 findText값이 있을시 해당 항목으로 변경
		...
		ArrayList<ProductVO> pList = pDAO.selectByTitleDB(findText, startListNum, endListNum);// 원하는 구간의 db를 출력
