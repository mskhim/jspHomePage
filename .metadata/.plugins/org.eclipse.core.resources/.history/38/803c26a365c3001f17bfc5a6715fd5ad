		...
		<section class="searchBar">
			<form method="post" action="<c:url value='/shopFindSelect.do?type=${param.type}' />">
				<label for="searchCriteria">검색 기준:</label> <select
					...
					placeholder="검색어를 입력하세요" value="${findText}" required>
				<button type="submit" class="searchButton">검색</button>
			<button type="button" class="searchButton"onclick="location.href='<c:url value="/shopSelect.do?pageNum=1&type=${param.type}" />'">검색취소</button>
			</form>
		</section>
