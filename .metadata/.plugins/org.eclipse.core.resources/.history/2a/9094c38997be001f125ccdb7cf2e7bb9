...
public class NoticeSelectAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws IOException {
    	...
    	//데이터베이스 에서 가져오는 값
    	int recordCount = nDAO.selectRecordDB(); //전체 리스트 개수 findText값이 있을시 해당 항목으로 변경
		...
		ArrayList<NoticeVO> nList = nDAO.selectDB(startListNum, endListNum);//원하는 구간의 db를 출력
		...
		ActionForward forward = new ActionForward("/home/notice/noticePage.jsp", false);
		return forward;
	}
